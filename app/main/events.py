...
def connect():
    """
    Sends a message to all users in the chatroom that a user joined the chat.
    """
    if current_user.is_authenticated:
        print(f"[EVENT] {current_user.username} has connected to the website.")
    else:
        send("[EVENT] Guest has joined the chat.")
...
def join_room(data):
    ...
    A query checks if the player is already in the room.
    """
    user = User.query.get(session["user_id"])
    room = Room.query.filter_by(name=data["room"]).first()

    if (
        db.session.query(room_users)
